...
# run: ./flow-antigravity-proposal.sh --task_id 1 --task_name "flow-antigravity-proposal" --query "Current realtime weather in Hamburg Germany please"
echo "🏃‍♂️  Flow from inference (antigravity) to proposal"
...
session_id=$(python trainer/inference_antigravity.py --task_id $TASK_ID --task_name $TASK_NAME --query "$QUERY" --system_prompt "Answer user question and finish task. Your answers must be based on true and reality, avoid answering that you do not know" --skills_dir ./workspace/skills --output_dir ./output --stream_mode | tail -n 1)
